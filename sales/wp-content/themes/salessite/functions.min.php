...
log_me("cookies after login:" . json_encode($_COOKIE));
 log_me("user cookie value after login:" . $_COOKIE['user_id']);
...
function salessite_scripts() {
	...
	if(is_user_logged_in () || !is_admin() ) { $status=false; }else{ $status=true; }
	...
	//wp_enqueue_script( 'picturefill', THEME_URL . '/js/picturefill.min.js', array(), '1.0.0', $status );	
	wp_enqueue_script( 'picturefillmain', BASE_CDN_URL . '/js/picturefill.js', array(), '1.0.0', $status );
	wp_enqueue_script( 'enterdirec', THEME_URL . '/app/preLogin.min.js', array(), '1.0.0', $status );
	if(is_front_page())
 ...
 */
   if(is_user_logged_in ()) { 
  		wp_enqueue_script( 'searchresult', THEME_URL . '/app/postLogin.min.js', array(), '1.0.0', $status );
   }		   
}
